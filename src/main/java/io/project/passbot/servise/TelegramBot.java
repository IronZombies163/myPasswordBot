...
import io.project.passbot.config.BotProperties;
import io.project.passbot.config.PassCreator;
...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    private final PassCreator passCreator;
    ...
    public TelegramBot(UserRepository userRepository, UserServise userServise, BotProperties config, PassCreator passCreator) {
            ...
            if (messageText.contains("/send") && botConfig.getOwner() == chatId) {
                ...
                switch (messageText) {
                    ...
                    case "сохранить":
                        savePassword(chatId);
                        break;
                    case "удаление паролей":
                        deleteMenuKeyboard(chatId, "Внимание! После удаления пароли нельзя восстановить =(");
                        break;

                    case "назад":
                        mainMenuKeyboard(chatId, okayEmoji);
                        break;
                    case "отмена":
                        deleteMenuKeyboard(chatId, checkMark);
                        break;
                    case "Отмена":
                        deleteMenuKeyboard(chatId, okayEmoji);
                        break;
                    case "перезаписать пароль":
                        rewritePassword(chatId);
                        break;
                    ...
                    case "сохраненные пароли":
                        ...
                        getSavePassword(chatId);
                        break;


                    case "удалить все пароли":
    ...
    private void registerUser(Message msg) {
        ...
        if (userRepository.findById(msg.getChatId()).isEmpty()) {
            ...
            user.setRegisterAt(new Timestamp(System.currentTimeMillis()).toLocalDateTime());
            user.setCountPass(0);
            userRepository.save(user);
        }
